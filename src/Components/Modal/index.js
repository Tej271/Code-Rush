...
export default function Modal() {
  ...
  // const renameProject = (proj_id) => {
  //   const temp = JSON.parse(localStorage.getItem("data"));
  //   let index = temp.findIndex((obj) => obj.id === proj_id);
  //   console.log(proj_id);
  //   temp[index].name = inp;
  //   // updateData(temp);
  // };
  ...
  return (
    <Transition.Root show={modalProps.show} as={Fragment}>
      <Dialog
        as="div"
              ...
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                ...
                <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                  <button
                    ...
                    onClick={() => {
                      if (modalProps.title === "Create New Project") {
                        addProject({ name: projectName });
                        closeModal();
                      } else if (modalProps.title === "Rename Project") {
                        renameProject({
                          id: modalProps.project_id,
                          newName: projectName,
                        });
                        closeModal();
                      } else if (modalProps.title === "Create New File") {
                        addFile({
                          id: modalProps.project_id,
                          name: projectName + ext,
                          language: lang,
                          lang_id: langId,
                        });
                        setExt("");
